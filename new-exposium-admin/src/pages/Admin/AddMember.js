...
const AddMember = () => {
  ...
  const [stallMember, setStallMember] = useState(true);
  ...
  const handleAddmember = async (e) => {
    ...
    try {
      ...
      await database
        .ref(`EXHIBITORS/${uid}/StallMember/${memeberData?.id}`)
        .set(memeberData?.id);
  ...
  const handleAddMemberDirectly = async (e) => {
    ...
    try {
      ...
      setShowAlert({
        msg: "Stall Member Added Successfully",
        isOpen: true,
        color: "success",
      });
    ...
    <Navigation>
      ...
      >
        <Button
          variant="contained"
          color="primary"
          onClick={() => setStallMember(!stallMember)}
        >
          {stallMember
